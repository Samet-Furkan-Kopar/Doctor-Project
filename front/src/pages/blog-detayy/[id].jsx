...
const ListingDynamicDetailsV1 = () => {
  ...
  const [sendMsgStatus, setSendMsgStatus] = useState(true)
    ...
    if (id) {
      filterService.getOneBlogWithId(id).then((res) => {
        ...
        if (res?.data?.succeded) {
          ...
          setSideBar(res?.data);
          res?.data?.data?.advertDetail?.isUserFavorite?.value  && setIsFavoriteStatus(true)
                ...
                <div className="col-lg-5 col-xl-4">
                  <div className="single_property_social_share position-static transform-none">
                    ...
                    <div className="spss style2 mt20 text-end tal-400"   >
                      <ul className="mb0">
                        {addFavoriteStatus &&
                          <li className="list-inline-item" style={{ background: isFavoriteStatus ? '#ff5a5f' : '' }}  onClick={async () => {
                            const addStatus = await addAdvertTofavorite(property?.advertDetail?._id.value)
                            setIsFavoriteStatus(!isFavoriteStatus)
                         
                          
                          }}>
                            <span className="flaticon-heart" style={{ color: isFavoriteStatus ? 'white' : '' }} ></span>
                          </li>
                        }
