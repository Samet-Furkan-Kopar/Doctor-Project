...
const CreateList = ({
        ...
        if (k === 'advertTypeName') {
          ...
          if (k != "coverPhoto") {
            if (typeof v.value == 'object') {
              if (crudFrom?.formValues[k]) {
                setFieldValue(k, crudFrom?.formValues[k])
              } else {
                setFieldValue(k, v.value.text)
                dispatch(updateField({ field: k, value: v.value.text }));

              }
              if (crudFrom?.formValues[`${k}_isStatus`]) {
                setFieldValue(k, crudFrom?.formValues[`${k}_isStatus`])
              } else {
                setFieldValue(`${k}_isStatus`, v.value.isStatus)
                dispatch(updateField({ field: `${k}_isStatus`, value: v.value.isStatus }));
              }
              ...
              if (crudFrom?.formValues[k]) {
                ...
                setFieldValue(k, v.value)
                dispatch(updateField({ field: k, value: v.value }));
            ...
            if (crudFrom?.formValues[k]) {
              setFieldValue(k, crudFrom?.formValues[k])
              setCoverImage([crudFrom?.formValues[k]])
            } else {
              setFieldValue(k, v.value)
              dispatch(updateField({ field: k, value: v.value }));
              setCoverImage([v.value])
            }
          }
        }
  ...
  const checkSquareMeterPrice = (sqr) => {
    if (values.advertPrice && sqr) {
      ...
      setFieldValue('squareMeterPrice', 1)
      dispatch(updateField({ field: "squareMeterPrice", value: 1 }));
    }
  }

  const getUpdateImage = (item) => {
    if (item) {
      if (typeof item == 'string') {
        ...
        return URL.createObjectURL(item[0])
      }
    } else {
      return '';
    }
                    ...
                    <li key={index} className="list-inline-item">
                      <div className="portfolio_item">
                        <Image
                          ...
                          src={getUpdateImage(item)}

                          alt="sozleşmeli emlak"
                        />
                        <div
