...
const FooterLinks = (props: any) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Footer Linkleri'));
    });
    ...
    const reloadLinkList = () => {
        footerManagementServices
            .getCategoryDetail(id, page, pageSize, search)
            .then((res:any) => {
    ...
    const destroyTargetLink = async (id: string) => {
        Swal.fire({
            icon: 'warning',
            ...
            if (result.value) {
                await footerManagementServices.deleteLink(id).then((res:any) => {
    ...
    const reOrderList = async (oldIndex: any, newIndex: any) => {
        ...
        await footerManagementServices.reorderLink(targetItem['_id'], {order: index}).then(res => {
    ...
    return (
        <div className="panel">
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white-light">Linkler</h5>
                <div>
                                        ...
                                        <ul className="flex items-center justify-left gap-2">
                                            ...
                                            <li>
                                                <Tippy content="Delete" >
                                                    <button type="button" onClick={() => destroyTargetLink(data._id)}>
            ...
            <UpdateLink updateLinkModal={updateLinkModal} categoryId={id} setUpdateLinkModal={setUpdateLinkModal} data={targetLink} reloadInitialValues={() => reloadLinkList()} />
            <AddNewLink addNewLinkModal={addNewLinkModal} categoryId={id} setAddNewLinkModal={setAddNewLinkModal} reloadInitialValues={() => reloadLinkList()} />
        </div>
    );
};
