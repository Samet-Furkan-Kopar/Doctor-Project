...
const UpdateLink = (props: any) => {
    ...
    const onSubmit = async (values: any) => {
        ...
        await navbarManagementServices.updateLink({ isVisible: values.isVisible.value, title: values.title, link: values.link.value }, props.data._id).then((res:any) => {
        ...
        props.setUpdateLinkModal(false);
    };

    return (
        <Transition appear show={props.updateLinkModal} as={Fragment}>
            <Dialog as="div" open={props.updateLinkModal} onClose={() => props.setUpdateLinkModal(false)} className="relative z-50">
                <Transition.Child
                    as={Fragment}
                    ...
                    <div className="flex min-h-full items-center justify-center px-4 py-8">
                        ...
                        >
                            <Dialog.Panel style={{ minHeight: '50%' }} className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                <button
                                    type="button"
                                    onClick={() => props.setUpdateLinkModal(false)}
                                ...
                                <div className="p-5">
                                    <Formik
                                        initialValues={{
                                            ...
                                            link: pageLinks.find((s: any) => s.value === props.data.link) || ''
                                        }}
